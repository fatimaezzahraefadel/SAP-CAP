    ...
    <div className="min-w-0">
      ...
      <main className="space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard icon={FilePlus2} label="A renouveler" value={alerts.length} detail="expire ou a 90 jours" />
        </section>
...
function LeaveForm({ form, setForm, types, editing, busy, onSubmit, onCancel }: { form: CreateDemandeCongeInput; setForm: (value: any) => void; types: Array<{ ID: string; libelle: string }>; editing: boolean; busy: boolean; onSubmit: (event: FormEvent) => void; onCancel: () => void }) {
  return <Card className="rounded-md"><CardHeader className="border-b pb-4"><CardTitle>{editing ? 'Modifier la demande' : 'Soumettre une demande'}</CardTitle><CardDescription>Le backend calcule la duree, le solde et les chevauchements.</CardDescription></CardHeader><CardContent className="pt-6"><form onSubmit={onSubmit} className="space-y-4"><Field label="Type de conge"><Select value={form.typeConge_ID} onValueChange={(value) => setForm((current: CreateDemandeCongeInput) => ({ ...current, typeConge_ID: value }))} required><SelectTrigger><SelectValue placeholder="Choisir un type" /></SelectTrigger><SelectContent>{types.map((item) => <SelectItem key={item.ID} value={item.ID}>{item.libelle}</SelectItem>)}</SelectContent></Select></Field><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><Field label="Date debut"><Input required type="date" value={form.dateDebut} onChange={(event) => setForm((current: CreateDemandeCongeInput) => ({ ...current, dateDebut: event.target.value }))} /></Field><Field label="Date fin"><Input required type="date" value={form.dateFin} onChange={(event) => setForm((current: CreateDemandeCongeInput) => ({ ...current, dateFin: event.target.value }))} /></Field></div><Field label="Motif"><Textarea value={form.motif} rows={3} onChange={(event) => setForm((current: CreateDemandeCongeInput) => ({ ...current, motif: event.target.value }))} /></Field><div className="flex gap-2"><Button type="submit" disabled={busy} className="flex-1 gap-2">{editing ? <Save className="h-4 w-4" /> : <Send className="h-4 w-4" />}{editing ? 'Enregistrer' : 'Soumettre'}</Button>{editing && <Button type="button" variant="outline" onClick={onCancel}>Annuler</Button>}</div></form></CardContent></Card>;
}
...
function CertificateForm({ form, setForm, domaines, editing, busy, onSubmit, onCancel }: { form: CreateCertificatInput; setForm: (value: any) => void; domaines: Array<{ ID: string; libelle: string }>; editing: boolean; busy: boolean; onSubmit: (event: FormEvent) => void; onCancel: () => void }) {
  return <Card className="rounded-md"><CardHeader className="border-b pb-4"><CardTitle>{editing ? 'Modifier le certificat' : 'Ajouter un certificat'}</CardTitle><CardDescription>Les certificats expires ou a renouveler sont signales.</CardDescription></CardHeader><CardContent className="pt-6"><form onSubmit={onSubmit} className="space-y-4"><Field label="Domaine"><Select value={form.domaine_ID} onValueChange={(value) => setForm((current: CreateCertificatInput) => ({ ...current, domaine_ID: value }))} required><SelectTrigger><SelectValue placeholder="Choisir un domaine" /></SelectTrigger><SelectContent>{domaines.map((item) => <SelectItem key={item.ID} value={item.ID}>{item.libelle}</SelectItem>)}</SelectContent></Select></Field><Field label="Intitule"><Input required value={form.intitule} onChange={(event) => setForm((current: CreateCertificatInput) => ({ ...current, intitule: event.target.value }))} /></Field><Field label="Organisme"><Input value={form.organisme} onChange={(event) => setForm((current: CreateCertificatInput) => ({ ...current, organisme: event.target.value }))} /></Field><Field label="Identifiant"><Input value={form.identifiantCertificat} onChange={(event) => setForm((current: CreateCertificatInput) => ({ ...current, identifiantCertificat: event.target.value }))} /></Field><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><Field label="Obtention"><Input required type="date" value={form.dateObtention} onChange={(event) => setForm((current: CreateCertificatInput) => ({ ...current, dateObtention: event.target.value }))} /></Field><Field label="Expiration"><Input type="date" value={form.dateExpiration} onChange={(event) => setForm((current: CreateCertificatInput) => ({ ...current, dateExpiration: event.target.value }))} /></Field></div><Field label="Score"><Input value={form.score} onChange={(event) => setForm((current: CreateCertificatInput) => ({ ...current, score: event.target.value }))} /></Field><div className="flex gap-2"><Button type="submit" disabled={busy} className="flex-1 gap-2">{editing ? <Save className="h-4 w-4" /> : <FilePlus2 className="h-4 w-4" />}{editing ? 'Enregistrer' : 'Ajouter'}</Button>{editing && <Button type="button" variant="outline" onClick={onCancel}>Annuler</Button>}</div></form></CardContent></Card>;
}
